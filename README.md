...
- **Auth** — real signup/login/logout; streamer signup creates the account,
  ...
  Sessions work two ways: an httpOnly cookie, and an `Authorization: Bearer`
  token returned at login/signup — so authentication also works in
  environments that block cookies (e.g. sandboxed preview iframes).
...
- **Messaging** — permanent database storage; Socket.IO realtime (no refresh);
  ...
  messages; per-conversation isolation.
