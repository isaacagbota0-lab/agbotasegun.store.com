...
async function render() {
  ...
  setRealtimeHandler((evt) => {
    if (evt.type === 'message:new') {
      ...
      if (chat.convId && chat.opened && evt.msg.conversation_id === chat.convId && evt.msg.sender_id !== store.user.id) {
        chat.messages.push(evt.msg);
        window.dispatchEvent(new CustomEvent('chat:message', { detail: evt.msg }));
        api(`/api/conversations/${chat.convId}/read`, { method: 'POST' }).catch(() => {});
      }
      updateNavBadges();
    }
