...
export const chatState = { filter: 'all', query: '', selected: null, messages: [], opened: false, sending: false, stick: true };
  ...
  window.addEventListener('chat:message', (e) => {
    ...
    if (chatState.opened && chatState.selected === m.conversation_id) {
      if (!chatState.messages.some((x) => x.id === m.id)) {
        ...
        if (btn) btn.classList.toggle('hidden', chatState.stick);
      }
    }
  });
...
export async function openConversation(convId) {
  ...
  main.innerHTML = `<div class="chat-head">
    ...
    <div class="chat-body-wrap">
      ...
      <button class="chat-scroll-btn hidden" id="chat-scroll-btn" title="Scroll to latest" aria-label="Scroll to latest message" style="transform:rotate(90deg)">${icon('arrowRight', 18)}</button>
    </div>
  ...
  try {
    ...
    if (body) {
      body.addEventListener('scroll', () => {
        ...
        chatState.stick = nearBottom;
        const btn = document.getElementById('chat-scroll-btn');
        if (btn) btn.classList.toggle('hidden', nearBottom);
      });
    }
    const scrollBtn = document.getElementById('chat-scroll-btn');
    if (scrollBtn) scrollBtn.addEventListener('click', () => { chatState.stick = true; scrollToBottom(); });
...
export function renderMessages() {
  ...
  body.innerHTML = html;
  // Images load asynchronously and change the scroll height — re-stick if the
  // user is at the bottom so the newest message stays visible.
  body.querySelectorAll('img').forEach((img) => {
    img.addEventListener('load', () => { if (chatState.stick) scrollToBottom(); });
  });
  if (chatState.stick) scrollToBottom();
}
