...
export async function signupView(el) {
  ...
  el.querySelector('#signup-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      store.user = d.user;
      connectSocket();
      try { await refreshAll(); } catch (_) { /* homepage renders from cached state */ }
      toastOk('Account created', `Welcome, ${d.user.name.split(' ')[0]} — you're logged in.`);
      // New streamers land on the public homepage so they can browse the
      // store, proof of work and support pages first. The dashboard stays
      // one click away via the "Dashboard" button.
      navigate('/');
    } catch (err) {
      btn.disabled = false;
